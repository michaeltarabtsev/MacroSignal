...
def fetch_headlines():
    if NEWS_API_KEY:
        try:
            ...
            hls  = [a["title"] for a in arts if a.get("title")]
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2 — TRADE SIGNAL GENERATOR
# ══════════════════════════════════════════════════════════════════════════════
elif section == "2️⃣  Trade Signal Generator":
    st.title("Section 2 — Trade Signal Generator")
    macro = st.session_state.macro_score
    if macro > 0.05:    regime_label, regime_color = "HAWKISH", "🟢"
    elif macro < -0.05: regime_label, regime_color = "DOVISH",  "🔴"
    else:               regime_label, regime_color = "NEUTRAL",  "🔵"
    st.caption(f"Macro regime from Section 1: {regime_color} **{regime_label}** (score: {macro:+.3f})")
    if macro == 0.0:
        st.warning("Run Section 1 first to set the macro regime. Defaulting to NEUTRAL.")

    if st.button("Fetch Live Headlines 📡"):
        with st.spinner("Fetching..."):
            headlines, is_live = fetch_headlines()
            st.session_state["headlines"]      = headlines
            st.session_state["headlines_live"] = is_live

    headlines = st.session_state.get("headlines", [])
    if headlines:
        badge = "🌐 Live" if st.session_state.get("headlines_live") else "📋 Demo"
        st.caption(f"Source: {badge}")
        selected = st.selectbox("Select a headline", headlines)
        st.session_state["selected_headline"] = selected

    headline_input = st.text_area(
        "Or paste any financial text",
    ...
    if st.button("Generate Trade Signal 🚀", type="primary"):
        if not headline_input.strip():
            ...
            zs_m, tok_m, fb_m = load_models()
            with st.spinner("Analysing..."):
                inputs = tok_m(headline_input, return_tensors="pt",
                               padding=True, truncation=True, max_length=512)
                with torch.no_grad():
                    logits = fb_m(**inputs).logits
                probs      = torch.nn.functional.softmax(logits, dim=-1).numpy()[0]
                confidence = float(max(probs))
                pos, neg   = float(probs[0]), float(probs[1])

            if confidence < CONFIDENCE_FLOOR:
                st.warning(f"Signal too weak ({confidence:.0%} confidence). No trade generated.")
            else:
                hl_sent = "positive" if pos > neg else "negative"
                if macro > 0.05:
                    direction = "SHORT" if hl_sent=="negative" else "LONG"
                    conviction = "HIGH" if hl_sent=="negative" else "LOW"
                    conf_boost = 1.15 if hl_sent=="negative" else 0.85
                elif macro < -0.05:
                    direction = "LONG" if hl_sent=="positive" else "SHORT"
                    conviction = "HIGH" if hl_sent=="positive" else "LOW"
                    conf_boost = 1.15 if hl_sent=="positive" else 0.85
                else:
                    direction = "LONG" if pos > neg else "SHORT"
                    conviction = "MODERATE"
                    conf_boost = 1.0
                adj_conf = min(confidence * conf_boost, 1.0)

                hl_lower = headline_input.lower()
                theme_scores = {k:0 for k in ASSET_POOL}
                for theme, data in ASSET_POOL.items():
                    for asset in data["Candidates"]:
                        if asset["Ticker"].lower().replace("=f","").replace("^","") in hl_lower:
                            theme_scores[theme] += 25
                        for kw in asset["Keywords"]:
                            if kw in hl_lower: theme_scores[theme] += 10
                        if macro != 0 and asset["regime_bias"] == ("hawkish" if macro>0 else "dovish"):
                            theme_scores[theme] += 8
                best_theme = max(theme_scores, key=theme_scores.get)
                if theme_scores[best_theme]==0: best_theme="BROAD_MARKET"
                theme_data = ASSET_POOL[best_theme]

                scored = []
                for asset in theme_data["Candidates"]:
                    sc = sum(10 for kw in asset["Keywords"] if kw in hl_lower)
                    sc += 50 if asset["Ticker"].lower().replace("=f","") in hl_lower else 0
                    if asset["regime_bias"]==("hawkish" if macro>0 else "dovish"): sc+=20
                    scored.append((sc, asset))
                scored.sort(key=lambda x: x[0], reverse=True)
                top_assets = [a for _,a in scored[:3]]

                dir_c  = "green" if direction=="LONG" else "red"
                conv_c = "green" if conviction=="HIGH" else ("orange" if conviction=="MODERATE" else "red")
                st.markdown(f"### :{dir_c}[{direction}] — :{conv_c}[{conviction} conviction]")
                st.caption(f"Asset class: **{theme_data['Class']}** | Confidence: {adj_conf:.0%}")
                ...
                for i, asset in enumerate(top_assets):
                    ...
                    wt, notional, friction = calculate_position_size(ticker, adj_conf)
                    with cols[i]:
                        st.metric(f"**{ticker}**", f"${entry:,.2f}")
                        st.caption(asset["Desc"])
                        st.markdown(f"🎯 Target: **${tp:,.2f}**")
                        st.markdown(f"🛑 Stop: **${sl:,.2f}**")
                        st.markdown(f"💰 Size: **${notional:,.0f}** ({wt}%)")
                        if asset["regime_bias"]==("hawkish" if macro>0 else "dovish"):
                            st.success("✓ regime aligned")
                    logged_rows.append([time.strftime("%Y-%m-%d %H:%M:%S"),
                                        ticker,direction,entry,wt,notional,friction])
                try:
                    initialise_ledger()
                    with open(DB_FILE, mode="a", newline="") as f:
                        csv.writer(f).writerows(logged_rows)
                    st.success(f"✅ {len(logged_rows)} positions logged.")
                except Exception as e:
                    st.error(f"Could not write to ledger: {e}")
                st.caption("⚠️ For educational purposes only. Not financial advice.")

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — POSITION JOURNAL
# ══════════════════════════════════════════════════════════════════════════════
elif section == "3️⃣  Position Journal":
    st.title("Section 3 — Position Journal & Live P&L")
    st.caption("Prices as of last available market close")
    ...
    df = load_ledger()
    if df.empty:
        st.info("No trades found. Run Section 2 to generate signals first.")
    else:
        tickers = ["ALL"] + sorted(df["Ticker"].dropna().unique().tolist())
        with c1: filter_ticker = st.selectbox("Filter by ticker", tickers)
        with c2: filter_dir    = st.selectbox("Filter by direction", ["ALL","LONG","SHORT"])

        filtered = df.copy()
        if filter_ticker != "ALL": filtered = filtered[filtered["Ticker"]==filter_ticker]
        if filter_dir    != "ALL": filtered = filtered[filtered["Direction"]==filter_dir]

        if filtered.empty:
            st.warning("No trades match the selected filters.")
        else:
            blocks = (filtered.groupby("Timestamp").first().reset_index()
            ...
            for _, block_meta in blocks.iterrows():
                ...
                total_n, total_c, total_f = 0.0, 0.0, 0.0
                rows_list = []
                for _, trade in block_trades.iterrows():
                    ...
                    cur, pnl_usd, pnl_pct = calc_pnl(entry, live, notional, direction)
                    stop_dist, near_stop   = stop_distance(entry, live, direction)
                    total_n += notional; total_c += cur; total_f += friction
                    rows_list.append({
                        "Ticker": ticker, "Dir": direction,
                        "Entry": f"${entry:,.2f}", "Live": f"${live:,.2f}",
                        "P&L %": f"{pnl_pct:+.2f}%", "P&L $": f"${pnl_usd:+,.2f}",
                        "Stop dist": f"{stop_dist:+.2f}%",
                        "Alert": "🔴 NEAR STOP" if near_stop else "✅",
                        "Open": time_open(ts),
                        "Size": f"${notional:,.0f} ({weight}%)"
                    })
                net_pnl = (total_c - total_n) - total_f
                net_pct = net_pnl / total_n * 100 if total_n > 0 else 0
                c_label = "green" if net_pct >= 0 else "red"
                with st.expander(
                    f"Block {ts}  |  {block_dir}  |  :{c_label}[{net_pct:+.2f}%]  |  ${total_n:,.0f}",
                    expanded=True
                ):
                    m1, m2, m3 = st.columns(3)
                    m1.metric("Block Size", f"${total_n:,.0f}")
                    m2.metric("Net P&L", f"{net_pct:+.2f}%", f"${net_pnl:+,.2f}")
                    m3.metric("Friction drag", f"-${total_f:,.2f}")
                    st.dataframe(pd.DataFrame(rows_list),
                                 use_container_width=True, hide_index=True)

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 4 — PORTFOLIO DASHBOARD
# ══════════════════════════════════════════════════════════════════════════════
elif section == "4️⃣  Portfolio Dashboard":
    st.title("Section 4 — Portfolio Analytics Dashboard")
    st.caption("All figures after friction costs · Prices as of last available market close")
    ...
    if df.empty:
        ...
        live_prices = {tk: get_live_price(tk) for tk in df["Clean_Ticker"].unique()}
        ...
        for _, row in df.iterrows():
            ...
            cur, pnl_usd, _ = calc_pnl(entry, live, notional, direction)
            rows.append({"Timestamp":row["Timestamp"],"Ticker":ticker,
                         "Notional":notional,"Friction":friction,"PnL_USD":pnl_usd,"Current":cur})

        df_live = pd.DataFrame(rows)
        df_live["Timestamp"] = pd.to_datetime(df_live["Timestamp"])
        df_live = df_live.sort_values("Timestamp")

        total_n   = df_live["Notional"].sum()
        total_f   = df_live["Friction"].sum()
        net_pnl   = df_live["PnL_USD"].sum() - total_f
        net_pct   = (net_pnl / total_n * 100) if total_n > 0 else 0

        daily_pnl = df_live.groupby(df_live["Timestamp"].dt.date)["PnL_USD"].sum().values
        if len(daily_pnl)>1 and daily_pnl.std()>0:
            sharpe = float((daily_pnl.mean()/daily_pnl.std())*np.sqrt(252))
            sharpe_str = f"{sharpe:.2f}"
        else:
            sharpe_str = "N/A (need >1 day)"

        cum      = df_live["PnL_USD"].cumsum().values
        run_max  = np.maximum.accumulate(cum)
        ddowns   = cum - run_max
        max_dd   = float(ddowns.min())
        max_dd_p = (max_dd/total_n*100) if total_n>0 else 0
        worst    = float(df_live["PnL_USD"].min())
        worst_tk = df_live.loc[df_live["PnL_USD"].idxmin(),"Ticker"]

        c1,c2,c3 = st.columns(3)
        c1.metric("Signal Blocks", df_live["Timestamp"].nunique(),
                  f"{len(df_live)} positions")
        c2.metric("Total Notional", f"${total_n:,.0f}")
        c3.metric("Net P&L", f"{net_pct:+.2f}%", f"${net_pnl:+,.2f}")
        c4,c5,c6 = st.columns(3)
        c4.metric("Sharpe (Ann.)", sharpe_str)
        c5.metric("Max Drawdown", f"{max_dd_p:+.2f}%", f"${max_dd:+,.2f}")
        c6.metric("Largest Loss", f"${worst:+,.2f}", worst_tk)
        ...
        ax1.plot(times, cum_pnl, color="#38bdf8", lw=2, label="Portfolio")
        ax1.fill_between(times,cum_pnl,0,where=(cum_pnl>=0),alpha=0.15,color="#10b981")
        ax1.fill_between(times,cum_pnl,0,where=(cum_pnl<0), alpha=0.15,color="#f43f5e")
        try:
            start = df_live["Timestamp"].min().strftime("%Y-%m-%d")
            spy_r = yf.Ticker("SPY").history(start=start,period="1mo")["Close"].dropna()
            if len(spy_r)>=2:
                spy_cum = ((1+spy_r.pct_change().dropna()).cumprod()-1)*total_n
                ax1.plot(spy_cum.index,spy_cum.values,color="#f59e0b",lw=1.5,ls="--",
                         alpha=0.8,label="SPY (scaled)")
        except Exception:
            pass
        ax1.axhline(0,color="#64748b",lw=1,ls="--")
        ax1.set_title("Cumulative P&L vs SPY Benchmark",fontsize=11,fontweight="bold",color="#38bdf8")
        ax1.legend(fontsize=9,facecolor="#1e293b",edgecolor="#334155",labelcolor="#e2e8f0")
        ax1.grid(axis="y",ls=":",alpha=0.4)
        for sp in ax1.spines.values(): sp.set_visible(False)

        apnl   = df_live.groupby("Ticker")["PnL_USD"].sum().sort_values()
        colors = ["#10b981" if v>=0 else "#f43f5e" for v in apnl.values]
        ax2.barh(apnl.index,apnl.values,color=colors,edgecolor="#334155",height=0.55)
        ax2.axvline(0,color="#64748b",lw=1,ls="--")
        ax2.set_title("P&L by Asset",fontsize=11,fontweight="bold",color="#38bdf8")
        ax2.grid(axis="x",ls=":",alpha=0.4)
        for sp in ax2.spines.values(): sp.set_visible(False)

        ax3.fill_between(range(len(ddowns)),ddowns,0,alpha=0.6,color="#f43f5e")
        ax3.plot(ddowns,color="#f43f5e",lw=1.5)
        ax3.axhline(0,color="#64748b",lw=1,ls="--")
        ax3.set_title("Drawdown from Peak",fontsize=11,fontweight="bold",color="#38bdf8")
...
# ── Section 5: Backtest results (shown only on Section 1 page as context) ──────
if section == "1️⃣  Macro Regime Analyser":
    pass  # Section 5 block follows below
